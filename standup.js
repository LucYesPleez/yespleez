...
function _renderStandupAvailSummary(rows) {
  ...
  if (!el) return;
  if (!rows.length) {
    el.innerHTML = 'No availability set — tap <strong style="color:#E879F9;">MANAGE</strong> to add open dates.';
    return;
  }
  const today = new Date().toISOString().split('T')[0];
  const upcoming = rows
  ...
  el.innerHTML = `<div style="display:flex;flex-wrap:wrap;gap:8px;">
    ${upcoming.map(r => {
      ...
      return `<div style="background:rgba(232,121,249,.1);border:1px solid rgba(232,121,249,.3);border-radius:8px;padding:5px 12px;font-size:12px;font-family:'Bebas Neue',sans-serif;letter-spacing:1px;color:#E879F9;">${label.toUpperCase()}</div>`;
    }).join('')}
    ...
    overlay.innerHTML = `
      <div style="background:#0f0f1a;border-radius:20px 20px 0 0;width:100%;max-width:480px;padding:24px 20px 40px;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;">
          <div style="font-family:'Bebas Neue',sans-serif;font-size:22px;letter-spacing:2px;color:#E879F9;">ACT AVAILABILITY</div>
...
function _renderStandupAvailGrid() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    if (isAvail) { bg = 'rgba(232,121,249,.18)'; border = '1px solid rgba(232,121,249,.5)'; color = '#E879F9'; }
