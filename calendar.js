...
function calSlimDayCard(ev) {
  ...
  return `<div onclick="calOpenEvent('${ev.id}')" style="border-radius:14px;overflow:hidden;cursor:pointer;border:1px solid rgba(255,255,255,.07);position:relative;height:calc((100dvh - 240px) / 3.5);min-height:140px;background:${bg};touch-action:manipulation;flex-shrink:0;">
...
const _rolePillStyle = {
  artist:  { bg:'#00E5FF', dark:true  },
  host:    { bg:'#FF3399', dark:false },
  band:    { bg:'#FF8C42', dark:true  },
  venue:   { bg:'#00E5A0', dark:true  },
  standup: { bg:'#FF88AA', dark:true  },
};
...
function _calProfileCard(p) {
  ...
  return `<div class="cal-small-card" onclick="showProfilePage && showProfilePage('${userId}')" style="background:${bg};flex-shrink:0;position:relative;border:none;">
    ...
    <div class="cal-small-card-content">
      ...
      ${p.sound ? `<div style="font-size:10px;color:rgba(255,255,255,.5);margin-top:5px;line-height:1.4;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">${esc(p.sound)}</div>` : ''}
    </div>
  </div>`;
}
