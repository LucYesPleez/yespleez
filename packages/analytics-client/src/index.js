/**
 * @yespleez/analytics-client — THE platform collector, shared.
 *
 * Extracted VERBATIM from Scene's lib/analytics.js (Analytics v2,
 * Phase G). One change only: the Supabase client and the suburb map
 * arrive by INJECTION, not import.
 *
 * ⛔⛔ THIS PACKAGE NEVER CREATES A SUPABASE CLIENT. A second GoTrue
 * client on one storage key destroys the owner's session — the
 * consumer passes ITS client to configureAnalytics() before first use,
 * and the whole module treats it as opaque. A consumer that forgets is
 * safe anyway: every call is fire-and-forget and every error is
 * swallowed (rule 1 below), so an unconfigured collector records
 * nothing and breaks nothing.
 *
 * Storage keys (yp_device_id, yp_session_id, …) are UNCHANGED from the
 * Scene original, so every existing device identity survives the
...
/** Called once by the consumer, before its first tracked event. */
...
export function normaliseRegion(input) {
  try {
    ...
    const codes = deps.suburbMap[t];                // EXACT match only
...
export function track(name, props = {}) {
  try {
    ...
    deps.supabase.from('usage_events').insert(row).then(
      ({ error }) => {
...
function recordAttributionTouch() {
  try {
    ...
    deps.supabase.from('attribution_touches').insert({
      device_id: deviceId(),
      session_id: sessionId(),
      user_id: currentUserId,
      ...parsed,
