...
>
> **Ratified 20 Jul 2026.** This document is binding. It is the architectural specification M8
> implements.
>
...
>
> **Rule numbers `C1`–`C32` and `D1`–`D17` are frozen** — a stable citation interface. They may
...
>
> **This Markdown file is the canonical, authoritative artifact** — unlike the four HTML documents,
> which were authored in HTML and whose `.md` files are lossy derivatives. This document was
> authored in Markdown, so the Markdown *is* the authored record. Converting it to HTML would make
> the derived file canonical, inverting the principle the rule protects (see
> [`README.md`](README.md)).
>
> Status: **RATIFIED v1.0** — 20 Jul 2026 · frozen. Full amendment record at **§13**.
>
> **Amendments** are made by minting a new version (`communication-v1.1`), never by editing this
> file — the same rule that makes `identity-v1.1` a separate file from `identity-v1.0`.
...
as a layer above it, the voice pipeline, the storage model, and the decisions deferred to later
revisions.

**Status** CANONICAL — ratified and frozen 20 Jul 2026 **Governed by** Identity v1.1 §A5 (frozen)
**Implements** M8 **Implementation prerequisites** M6 · M7 (§0)
...
5. **Promoted to canonical and added to the canonical table**, and removed from README "Drafts".

   > **Amended 20 Jul 2026.** Originally *"re-authored as `communication-v1.0.html`"*. That assumed
   > this directory's HTML-authoring workflow, which does not apply here: the four HTML documents
   > were **authored** in HTML and their `.md` files are lossy derivatives generated by
   > `render-markdown.js`. This document was authored in Markdown. Converting it to HTML would make
   > the **derived** artifact canonical — inverting the principle the "HTML is authoritative" rule
   > exists to protect, which is that **the authored artifact is authoritative**. The toolchain also
   > runs only HTML→Markdown, and hand-authoring the HTML is precisely what README forbids:
   > *"Nobody should ever retype a frozen document by hand — that is how paraphrase enters a
   > canonical record."*
